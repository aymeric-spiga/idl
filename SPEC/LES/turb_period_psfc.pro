...
filefile = "/home/aslmd/bigLES10m/psfc.nc"
...
getcdf, file=filefile, charvar="PSFC", invar=zevar
        ...
        nx = n_elements(zevar(*,0,0))
        ny = n_elements(zevar(0,*,0))
        ntot = n_elements(zevar(0,0,*))
        ;lon = findgen(nx) / 10.
        ;lat = findgen(ny) / 10.
	;;lon = findgen(2*nx+1) / 10.
	;;lat = findgen(2*ny+1) / 10.
        lon = findgen(2*nx+1) * dx ;/ 1000.
        lat = findgen(2*ny+1) * dx ;/ 1000.
    lon = findgen(nx) * dx ;/ 1000.
    lat = findgen(ny) * dx ;/ 1000.
...
title_user='Surface pressure (Pa) / Local time = '+string(start_lt + freq_out*float(nt)/3700.,'(F6.3)')
print, title_user
;
;
;
