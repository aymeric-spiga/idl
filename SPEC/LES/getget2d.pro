function getget2d, name, charvar, count=count, offset=offset, stride=stride, coordmean=coordmean, anomaly=anomaly
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; name: char
; charvar: char
...
ENDIF ELSE BEGIN
  ...
  NCDF_DIMINQ, id, NCDF_DIMID(id, 'Time'         ), toto, titi & var(2) = titi 
  ;print, var

  ;
  ; define default data windows
  ;
  if (n_elements(offset) le 3) then offset=[0,0,0]
  if (n_elements(stride) le 3) then stride=[1,1,1]
  if (n_elements(count) le 3) then count=intarr(3)
